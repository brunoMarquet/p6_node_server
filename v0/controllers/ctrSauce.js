...
exports.getAllSauceByUser = (req, res, next) => {
  ...
  if (res.locals.userId) {
    ...
    Sauce.find()
      .then((sauces) => {
        if (action === "creer") {
          const lesSauces = findByUserCreer(idUser, sauces);
...
function findByUserCreer(idUser, sauces) {
  let lesSauces = [];
  for (sauce of sauces) {
...
/**fonction modifySauceLight sans prise en compte de l'image  */
...
/*
exports.getAllSauceByUser_2 = (req, res, next) => {
  const token2 = req.headers.authorization.split("Action")[1];
  console.log("ok_", token2);
